"""UpdateCasePriorityOrganizationCasePrioritiesCasePriorityIdPut tool implementation."""
import json
import logging
...
@mcp.tool()
def updatecasepriorityorganizationcaseprioritiescasepriorityidput(
    ...
    if not case_priority_id:
        return json.dumps({"error": "Missing required path parameter: case_priority_id"})
    if not organization:
    ...
    # Build URL
    url = f"{config['base_url']}/{organization}/case_priorities/{case_priority_id}"

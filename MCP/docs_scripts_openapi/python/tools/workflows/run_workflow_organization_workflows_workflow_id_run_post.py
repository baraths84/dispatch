"""RunWorkflowOrganizationWorkflowsWorkflowIdRunPost tool implementation."""
import json
import logging
...
@mcp.tool()
def runworkfloworganizationworkflowsworkflowidrunpost(
    workflow_id: int,
    organization: str,
    artifacts: list = None,
    case: dict = None,
    ...
    if not workflow_id:
        return json.dumps({"error": "Missing required path parameter: workflow_id"})
    if not organization:
        return json.dumps({"error": "Missing required path parameter: organization"})
    # Build request body
    request_body = {}
    if artifacts is not None:
        request_body["artifacts"] = artifacts
    if case is not None:
    ...
    if run_reason is not None:
        request_body["run_reason"] = run_reason
    ...
    # Build URL
    url = f"{config['base_url']}/{organization}/workflows/{workflow_id}/run"
